...
Tracer::Tracer(std::shared_ptr<TracePublisher> &publisher)
    : get_time_(getRealTime), get_id_(getId), sampler_(KeepAllSampler()) {
  auto writer = std::make_shared<ExternalWriter>();
  std::cerr << writer->publisher() << std::endl;
  publisher = writer->publisher();
  std::cerr << publisher << std::endl;
